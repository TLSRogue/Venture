...
export function spawnArenaBoss(party, round) {
  ...
  if (bossCard.isBattleBrother) {
    ...
    sharedState.zoneCards = [rhino, getZoneAreaCard('arena', 1), tiger];
...
export const registerAdventureHandlers = (io, socket) => {
  socket.on('party:enterZone', (zoneName) => {
    ...
    if (zoneName === 'arena') {
      ...
      const bossPool = party.sharedState.zoneDeck.filter((c) => c.arenaReward).map((c) => ({ ...c })); // Deep clone each boss template
  ...
  socket.on('party:playerAction', async (action) => {
    ...
    try {
      if (action.type === 'resolvePvpFlee') {
        ...
        if (action.payload.allow) {
          ...
          party.sharedState.log.push({
            message: `You have shown mercy. The other party has returned home.`,
            type: 'info',
          });
          opponentParty.sharedState.log.push({
            message: `Your plea was accepted! The encounter ends peacefully.`,
            type: 'success',
          });
          ...
          if (encounter.isDuel) {
            ...
            party.sharedState.log.push({
              message: 'Combat has ended! You may continue your adventure.',
              type: 'success',
            });
      ...
      if (party.sharedState.pvpEncounterId) {
        ...
        if (!encounter) {
          console.log(
            `[playerAction] BLOCKED: No encounter found for pvpEncounterId ${party.sharedState.pvpEncounterId}`
          );
          return;
        }
      ...
      switch (action.type) {
        ...
        case 'learnTrainingSpell': {
          ...
          // Already known check
          const alreadyKnown =
            character.spellbook.some((s) => s.name === spellName) ||
            character.equippedSpells.some((s) => s.name === spellName);
          if (alreadyKnown) {
          ...
          party.sharedState.log.push({
            message: `${character.characterName} has learned ${spellData.icon} ${spellData.name}!`,
            type: 'success',
          });
        ...
        case 'campfireCraft': {
          ...
          if (playerHasMaterials(campfireChar, recipe.materials)) {
            ...
            if (baseItem) {
              ...
              party.sharedState.log.push({
                message: `${campfireChar.characterName} cooks ${baseItem.icon || ''} ${recipe.result.name} over the Campfire!`,
                type: 'success',
              });
            }
          } else {
        ...
        case 'compostItem': {
          ...
          if (slopItemBase) {
            ...
            party.sharedState.log.push({
              message: `${compostChar.characterName} composted ${targetItem.name} and received ${slopItemBase.icon} Slop!`,
              type: 'info',
            });
          }
        ...
        case 'endTurn':
          if (party.sharedState.pvpEncounterId) {
            ...
            party.sharedState.log.push({
              message: `${player.character.characterName} has ended their turn.`,
              type: 'info',
            });
