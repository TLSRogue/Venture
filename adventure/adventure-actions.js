...
import {
  ...
  dispatchSpecialSpell,
  handleZoneEffectSpell,
  resolveSpellRollAndConsume,
  handleMonkFocusGain,
  checkSpellPvpReaction,
  applySpellHeal,
  applySpellBuff,
  applySpellDebuff,
  applySpellAttack,
  handleCooldownReset,
} from './spell-cast-helpers.js';
...
export async function processWeaponAttack(io, party, player, payload) {
  ...
  if (isPvP && target.isPlayer) {
    ...
    modifyThreat(actingPlayerState, 1);
  ...
  if (attackResult.modifiers.focusModifier !== 0) {
    log.push({
      message: `${character.characterName} is focused! (+${attackResult.modifiers.focusModifier} to attack roll)`,
      type: 'info',
    });
  }
  ...
  if (attackResult.isCriticalFail) {
    ...
    if (target.state && target.state.darknessShrouded) {
      ...
      if (!hasLight) {
        log.push({
          message: `${target.name} is shrouded in darkness! You cannot target them without a light source!`,
          type: 'info',
        });
        broadcastAdventureUpdate(io, party);
        await checkAndEndTurnForPlayer(io, party, player);
    ...
    if (
      weapon.range === 'melee' ||
      (!weapon.range && !['Staff', 'Two-Hand Bow', 'One-Hand Crossbow', 'Wand'].includes(weapon.weaponType))
    ) {
...
export async function processEquipItem(io, party, player, payload) {
  ...
  if (party.sharedState) {
    ...
    const logTarget = party.sharedState.pvpEncounterId
      ? pvpEncounters[party.sharedState.pvpEncounterId]
      : party.sharedState;
  ...
  if (itemToEquip.hands === 2) {
    ...
    if (character.equipment.offHand && character.equipment.offHand !== character.equipment.mainHand)
      itemsToUnequip.push(character.equipment.offHand);
  } else {
    if (
      ['mainHand', 'offHand'].includes(chosenSlot) &&
      character.equipment.mainHand &&
      character.equipment.mainHand.hands === 2
    ) {
...
export async function processUseItemAbility(io, party, player, payload) {
  ...
  if (
    !item ||
    !item.activatedAbility ||
    !actingPlayerState ||
    actingPlayerState.actionPoints < item.activatedAbility.cost ||
    (actingPlayerState.itemCooldowns[item.name] || 0) > 0
  ) {
    return;
  }
  ...
  if (ability.buff) {
    ...
    logTarget.log.push({
      message: `${character.characterName} used ${ability.name} and gained the ${buff.type} buff!`,
      type: 'heal',
    });
  }
  if (ability.effect === 'cleanse') {
    ...
    if (poisonIndex !== -1) {
      ...
      logTarget.log.push({
        message: `${character.characterName} used ${ability.name}, but there was nothing to cleanse.`,
        type: 'info',
      });
    }
  }
  ...
  if (item.targetEnemy) {
    ...
    if (item.createsZoneEffect === 'campfire') {
      ...
      if (existingCampfire) {
        ...
        logTarget.log.push({
          message: `${character.characterName} stokes the Campfire! (Refreshed to 5 turns)`,
          type: 'success',
        });
      } else {
      ...
      if (item.heal) {
        ...
        logTarget.log.push({
          message: `${character.characterName} used ${item.name}, healing for ${item.heal} HP.`,
          type: 'heal',
        });
      }
      if (item.buff) {
...
export async function processUnequipItem(io, party, player, payload) {
  ...
  if (party.sharedState) {
    ...
    const logTarget = party.sharedState.pvpEncounterId
      ...
      : party.sharedState;
    logTarget.log.push({
      message: `${character.characterName} spends 1 AP to unequip ${itemToUnequip.name}.`,
      type: 'info',
    });
  }
