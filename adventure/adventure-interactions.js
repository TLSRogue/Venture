...
export function processDropItem(io, party, player, payload) {
  ...
  if (itemToDrop) {
    ...
    sharedState.log.push({
      message: `${character.characterName} dropped ${itemToDrop.name} to the ground.`,
      type: 'info',
    });
...
export function processTakeGroundLoot(io, party, player, payload) {
  ...
  if (itemToTake) {
    if (addItemToInventoryServer(character, itemToTake)) {
      ...
      sharedState.log.push({
        message: `${character.characterName} tried to pick up ${itemToTake.name}, but their inventory is full.`,
        type: 'damage',
      });
      broadcastAdventureUpdate(io, party);
    }
...
export function processTakeAllGroundLoot(io, party, player) {
  ...
  if (itemsPickedUp > 0) {
    sharedState.log.push({
      message: `${character.characterName} picked up ${itemsPickedUp} item${itemsPickedUp > 1 ? 's' : ''} from the ground.`,
      type: 'success',
    });
  }
  ...
  if (inventoryFull && sharedState.groundLoot.length > 0) {
    sharedState.log.push({
      message: `${character.characterName}'s inventory is full. ${sharedState.groundLoot.length} item${sharedState.groundLoot.length > 1 ? 's' : ''} remain on the ground.`,
      type: 'damage',
    });
  }
...
export async function processInteractWithCard(io, party, player, payload) {
  ...
  if (card.name === 'The Mansion') {
    sharedState.log.push({
      message: 'The party enters the decrepit mansion. A chill runs down their spines...',
      type: 'info',
    });
  ...
  if (card.type === 'resource') {
    ...
    const hasTool =
      character.inventory.some(hasValidTool) ||
    ...
    if (isSuccess) {
      ...
      if (lootItemData) {
        if (!addItemToInventoryServer(character, lootItemData, 1, sharedState.groundLoot)) {
          sharedState.log.push({
            message: `Success! But their inventory is full. They dropped 1 ${lootItemData.name} on the ground.`,
            type: 'damage',
          });
        } else {
    ...
    if (card.charges <= 0) {
      ...
      if (card.onDepletedSpawn && sharedState.currentZone === 'goblinCaves') {
        ...
        if (spawnRoll <= 50) {
          ...
          const enemyCards = goblinCavesPool.filter(
            (entry) => entry.card.type === 'enemy' && entry.card.name !== 'Gorbon the Goblin King'
          );
          ...
          if (treasureChestEntry) {
            ...
            sharedState.log.push({
              message: `A hidden treasure chest was revealed behind the boulders!`,
              type: 'success',
            });
          } else {
            sharedState.zoneCards[cardIndex] = getZoneAreaCard(sharedState.currentZone);
  ...
  // Interaction cards that spawn enemies (e.g., Chicken Coop -> Angry Rooster)
    ...
    if (enemyTemplate) {
      ...
      sharedState.zoneCards[cardIndex] = spawnedEnemy;
      sharedState.log.push({
        message: `${character.characterName} disturbed the ${card.name}! A ${spawnedEnemy.name} appears!`,
        type: 'damage',
      });
    }
  ...
  // NPCs are free to talk to
    ...
    if (card.type === 'treasure') {
      ...
      if (card.isArenaChest && sharedState.arenaState) {
        ...
        if (weaponItem) {
          if (addItemToInventoryServer(character, weaponItem, 1, sharedState.groundLoot)) {
            ...
            sharedState.log.push({
              message: `Found ${weaponItem.name}, but inventory was full. It was left on the ground.`,
              type: 'damage',
            });
          }
        }
        ...
        if (equipItem) {
          if (addItemToInventoryServer(character, equipItem, 1, sharedState.groundLoot)) {
            ...
            sharedState.log.push({
              message: `Found ${equipItem.name}, but inventory was full. It was left on the ground.`,
              type: 'damage',
            });
          }
        }
        ...
        if (foundItemsLog) {
          ...
          sharedState.log.push({
            message: `${character.characterName} opened the Arena Treasure Chest (Round ${arenaRound}) and found: ${foundItemsLog}!`,
            type: 'success',
          });
        }
        ...
        card.guaranteedCategories.forEach((category) => {
          ...
          if (randomItem) {
            if (addItemToInventoryServer(character, randomItem, 1, sharedState.groundLoot)) {
              ...
              sharedState.log.push({
                message: `Found ${randomItem.name}, but inventory was full. It was left on the ground.`,
                type: 'damage',
              });
            }
          }
        ...
        card.guaranteedItems.forEach((itemName) => {
          ...
          if (itemData) {
            if (addItemToInventoryServer(character, itemData, 1, sharedState.groundLoot)) {
              ...
              sharedState.log.push({
                message: `Found ${itemData.name}, but inventory was full. It was left on the ground.`,
                type: 'damage',
              });
            }
          }
      ...
      const lootTable = card.loot
        ? card.loot.map((item) => gameData.allItems.find((i) => i.name === item.name) || item)
        : !card.guaranteedCategories
          ? gameData.genericTreasureLoot.map((item) => gameData.allItems.find((i) => i.name === item.name) || item)
          : [];
        ...
        if (lootTable.length > 0) {
          ...
          if (randomLoot.gold) {
            ...
            if (addItemToInventoryServer(character, randomLoot, 1, sharedState.groundLoot)) {
              ...
              sharedState.log.push({
                message: `Found ${randomLoot.name}, but inventory was full. It was left on the ground.`,
                type: 'damage',
              });
            }
          }
      ...
      if (foundItemsLog) {
        foundItemsLog = foundItemsLog.slice(0, -2);
        sharedState.log.push({
          message: `${character.characterName} opened a chest and found: ${foundItemsLog}!`,
          type: 'success',
        });
      } else {
  ...
  if (npc.requiresClearArea && party.sharedState.zoneCards.some((c) => c && c.type === 'enemy')) {
    if (npc.dialogue && npc.dialogue.enemiesPresent && dialogueNodeKey === 'start') {
      ...
      party.sharedState.log.push({
        message: `You cannot interact with ${npc.name} while enemies are around!`,
        type: 'info',
      });
      return;
    }
...
export function processDialogueChoice(io, player, party, payload) {
  ...
  if (choice.questId) {
    ...
    if (questDetails) {
      if (choice.action === 'startDefenseQuest') {
        ...
        party.sharedState.log.push({
          message: `Defense Quest Started: Protect the Loyal Farmhand for ${party.sharedState.defenseQuest.maxTurns} turns!`,
          type: 'info',
        });
      } else {
      ...
      party.sharedState.log.push({
        message: `${character.characterName} accepted Quest: ${questDetails.title}`,
        type: 'success',
      });
    }
  }
  ...
  if (choice.questComplete) {
    ...
    if (questToComplete && questToComplete.status === 'readyToTurnIn') {
      ...
      io.to(player.id).emit('characterUpdate', character);
      party.sharedState.log.push({
        message: `${character.characterName} completed Quest: ${questToComplete.details.title}`,
        type: 'success',
      });
    }
  }
  ...
  if (choice.teachRecipe) {
    ...
    party.sharedState.log.push({
      message: `${npc.name} taught ${character.characterName} how to craft: ${recipeName}!`,
      type: 'success',
    });
  }
  ...
  if (choice.action === 'bribeCaptain') {
    if (character.gold < BRIBE_CAPTAIN_COST) {
      party.sharedState.log.push({
        message: `${character.characterName} doesn't have enough gold to bribe the captain!`,
        type: 'damage',
      });
      io.to(player.id).emit('party:hideDialogue');
      broadcastAdventureUpdate(io, party);
    ...
    io.to(player.id).emit('characterUpdate', character);
    party.sharedState.log.push({
      message: `${character.characterName} bribed the captain with ${BRIBE_CAPTAIN_COST} Gold!`,
      type: 'success',
    });
  ...
  if (choice.action === 'sneakAboard') {
    ...
    if (roll === 20) {
      party.sharedState.log.push({
        message: `${character.characterName} attempts to sneak aboard! ${rollDisplay} Success!`,
        type: 'success',
      });
      ...
      party.sharedState.log.push({
        message: `${character.characterName} attempts to sneak aboard! ${rollDisplay} Caught!`,
        type: 'damage',
      });
  ...
  if (choice.action === 'spiritCallBuff') {
    ...
    const oldBuffIndex = actingPlayerState.buffs.findIndex((b) =>
      ['Panther Spirit', 'Bear Spirit', 'Tree Spirit'].includes(b.type)
    );
    ...
    const spiritCallSpell =
      character.equippedSpells.find((s) => s.name === 'Spirit Call') ||
    ...
    if (choice.buff === 'Panther') {
      ...
      log.push({
        message: `${character.characterName} calls the Panther Spirit! (+${powerAmount} Agi)`,
        type: 'success',
      });
...
export function processLootPlayer(io, player, party, payload) {
  ...
  if (addItemToInventoryServer(lootingCharacter, itemToLoot, 1, sharedState.groundLoot)) {
    ...
    sharedState.log.push({
      message: `${lootingCharacter.characterName} looted ${itemToLoot.name} from ${deadPlayerState.name}'s bag.`,
      type: 'info',
    });
    ...
    sharedState.log.push({
      message: `${lootingCharacter.characterName} tried to loot, but their inventory is full. The item was left on the ground.`,
      type: 'damage',
    });
    broadcastAdventureUpdate(io, party);
  }
