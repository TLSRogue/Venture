...
export async function dispatchSpecialSpell(io, party, player, ctx) {
  ...
  if (result && result.pendingSelection) {
    ...
    log.push({
      message: `${character.characterName} prepares to cleanse ${result.pendingSelection.targetName}...`,
      type: 'info',
    });
    broadcastAdventureUpdate(io, party);
    return true;
  ...
  if (spell.type === 'expendHeat' && result && result.triggersAoe) {
    ...
    for (const hostileTarget of hostileTargets) {
      ...
      log.push({
        message: `${hostileTarget.name} takes ${dmg} Fire damage from the released heat! [id:${enemyId}]`,
        type: 'damage',
      });
...
export function resolveSpellRollAndConsume(ctx) {
  ...
  if (attackResult.modifiers.stealthModifier !== 0 && target)
    log.push({ message: `${target.name} is hidden in shadows! (-5 to hit)`, type: 'info' });
  if (attackResult.modifiers.focusModifier !== 0)
    log.push({
      message: `${character.characterName} is focused! (+${attackResult.modifiers.focusModifier} to spell roll)`,
      type: 'info',
    });
...
export function applySpellBuff(ctx) {
  ...
  if (buffTarget.applyBuff) {
    ...
    if (existingIndex !== -1) {
      if (buff.stackDuration) {
        ...
        log.push({
          message: `${buffTarget.name}'s ${buff.type} duration extended by ${buff.duration} turns!`,
          type: 'heal',
        });
        return;
      } else {
  ...
  if (buff.type === 'Magic Barrier') {
    ...
    log.push({
      message: `${buffTarget.name} gains Flame Shield (${buff.value} Fire Barrier)! Melee attackers will burn! [id:${tId}]`,
      type: 'heal',
    });
  } else if (buff.type === 'Ice Barrier') {
    log.push({
      message: `${buffTarget.name} gains Ice Barrier (${buff.value} Frost Barrier)! Melee attackers will be chilled! [id:${tId}]`,
      type: 'heal',
    });
  } else {
    log.push({
      message: `${buffTarget.name} gains ${buff.type}${buff.value ? ` (${buff.value})` : ''}! [id:${tId}]`,
      type: 'heal',
    });
  }
}
...
export function applySpellDebuff(ctx) {
  ...
  log.push({
    message: `${target.name} is now ${debuff.type.charAt(0).toUpperCase() + debuff.type.slice(1)}ed!`,
    type: 'damage',
  });
  return true;
}
...
export async function applySpellAttack(io, party, player, ctx) {
  ...
  if (spell.name === 'Whirlwind') {
    ...
    log.push({
      message: `${character.characterName} spins into a Whirlwind! ${numAttacks} total attacks!`,
      type: 'info',
    });
  }
  ...
  for (let attackNum = 0; attackNum < numAttacks; attackNum++) {
    ...
    if (spell.name === 'Whirlwind' && attackNum > 0) {
      currentAttackResult = resolveAttackRoll(
        actingPlayerState,
        character,
        null,
        spell.stat || 'strength',
      ...
      if (!currentAttackResult.isHit) {
        log.push({
          message: swingDescription + (currentAttackResult.roll === 1 ? ' Critical Miss!' : ' Miss!'),
          type: 'info',
        });
        broadcastAdventureUpdate(io, party);
    ...
    for (const t of uniqueTargets) {
        ...
        if (triggeredBombDrop && damageToDeal > 0) {
          ...
          if (emptySlotIndex !== -1) {
            const kegCard = {
              ...
              health: 6,
              overlayedCard:
                sharedState.zoneCards[emptySlotIndex] !== null ? sharedState.zoneCards[emptySlotIndex] : null,
            };
      ...
      if (spell.debuff) {
        ...
        const existingIndex = t.state.debuffs.findIndex(
          (d) => d.type.toLowerCase() === spell.debuff.type.toLowerCase()
        );
...
export function handleCooldownReset(ctx) {
  ...
  if (
    spell.resetCooldownThreshold &&
    attackResult &&
    attackResult.total >= spell.resetCooldownThreshold &&
    attackResult.isHit
  ) {
