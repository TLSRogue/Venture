...
export function getAvailablePlayerReactions(defendingCharacter, defendingPlayerState, attackDetails, log = null) {
  ...
  if (dodgeSpell && (defendingPlayerState.spellCooldowns[dodgeSpell.name] || 0) <= 0) {
    if (isWearingHeavy) {
      if (log)
        log.push({
          message: `${defendingPlayerState.name} could have Dodged, but their heavy gear prevented it!`,
          type: 'info',
        });
    } else {
  ...
  if (
    ...
    shield.type === 'shield' &&
    shield.reaction &&
    (defendingPlayerState.itemCooldowns[shield.name] || 0) <= 0
  ) {
  ...
  if (evasiveShotSpell && (defendingPlayerState.spellCooldowns[evasiveShotSpell.name] || 0) <= 0) {
    ...
    if (hasRangedWeapon) {
      if (isWearingHeavy) {
        if (log)
          log.push({
            message: `${defendingPlayerState.name} could have used Evasive Shot, but their heavy gear prevented it!`,
            type: 'info',
          });
      } else {
  ...
  if (parrySpell && (defendingPlayerState.spellCooldowns[parrySpell.name] || 0) <= 0) {
    ...
    if (isMeleeAttack && hasMeleeWeapon) {
      ...
      if (log)
        log.push({
          message: `${defendingPlayerState.name} could have Parried, but needs a melee weapon!`,
          type: 'info',
        });
    }
  }
...
export function checkVampirePhaseTransition(target, sharedState, gameData, log) {
  ...
  if (emptySlotIndex !== -1) {
    ...
    log.push({
      message: `The Vampire hisses in fury! "Assist me, minion!" A Vampire's Assistant emerges from the shadows!`,
      type: 'reaction',
    });
  }
}
