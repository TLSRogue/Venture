...
function handleCharacterUpdate(serverState) {
  ...
  if (activeSlotIndex !== null) {
    ...
    [
      ...
      'pvpEncounter',
      'pvpEncounterState',
    ].forEach((key) => delete stateToSave[key]);
...
function handlePartyAdventureUpdate(serverAdventureState) {
  ...
  if (
    document.getElementById('ground-loot-modal') &&
    !document.getElementById('ground-loot-modal').closest('.modal-overlay').classList.contains('hidden')
  ) {
    UIAdventure.showGroundLootModal();
  }
}
...
function handleTradeStart({ tradeId, otherPlayer, isPlayer1 }) {
  const initialTradeState = {
    id: tradeId,
    player1: {
      name: isPlayer1 ? gameState.characterName : otherPlayer,
      offer: { gold: 0, items: [] },
      locked: false,
      confirmed: false,
    },
    player2: {
      name: isPlayer1 ? otherPlayer : gameState.characterName,
      offer: { gold: 0, items: [] },
      locked: false,
      confirmed: false,
    },
...
function handleDuelEnd({ outcome, reward, finalLog }) {
  ...
  const message =
    outcome === 'win' ? `You are victorious! You won ${reward?.gold || 0} gold.` : 'You have been defeated!';
...
function handleLootRollStarted(lootData) {
  ...
  const rarityColor =
    { common: '#fff', uncommon: '#2ecc71', rare: '#3498db', quest: '#9b59b6' }[lootData.item.rarity] || '#fff';
...
function addEventListeners() {
  document.body.addEventListener('click', (e) => {
    ...
    if (target.closest('#finalize-char-btn'))
      return finalizeCharacterCreation(parseInt(target.closest('#finalize-char-btn').dataset.slot, 10));
    ...
    if (target.closest('.zone-card')) {
      ...
      if (zoneName === 'arena') {
        if (gameState.gold < ARENA_ENTRY_FEE)
          return UIMain.showInfoModal(`You don't have enough gold to enter the Arena! (Requires ${ARENA_ENTRY_FEE}G)`);
        ...
        const warningMessage =
          'You are about to enter The Blighted Wastes, a lawless PvP zone. If you are defeated by another player, you will lose ALL items in your inventory and everything you have equipped. Are you sure you wish to enter?';
    ...
    if (button) {
      ...
      if (button.id === 'copy-party-id-btn') {
        navigator.clipboard
          .writeText(document.querySelector('.party-id-display').textContent)
          .then(() => UIMain.showInfoModal('Party ID copied to clipboard!'));
        return;
      }
      ...
      if (button.matches('.tab, [data-tab-target]'))
        return UIPlayer.showTab(button.dataset.tab || button.dataset.tabTarget);
      ...
      if (button.id === 'end-turn-btn')
        return gameState.inDuel ? Network.emitDuelAction({ type: 'endTurn' }) : Combat.endTurn();
