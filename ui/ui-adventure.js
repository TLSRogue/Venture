...
const effectDefinitions = {
  ...
  frozen: {
    icon: '🧊',
    description: 'Frozen: Cannot take any actions. Some spells deal bonus damage to Frozen targets.',
  },
  ...
  'flame shield': {
    icon: '🛡️🔥',
    description: 'Flame Shield: Fire barrier that absorbs damage. Burns melee attackers.',
  },
  'ice barrier': {
    icon: '🛡️❄️',
    description: 'Ice Barrier: Frost barrier that absorbs damage. Chills melee attackers.',
  },
...
function addActionTooltipListener(element, itemOrSpell) {
  element.addEventListener('mousemove', (e) => {
    if (e.altKey) {
      ...
      showTooltip(
        `<strong>${itemOrSpell.name}</strong><br>${itemOrSpell.description}<br><em style='color: #aaa; font-size: 0.9em;'>Hold [Alt] for details</em>`
      );
    }
  });
  ...
  if (gameState && gameState.zoneEffects && gameState.zoneEffects.length > 0) {
    ...
    gameState.zoneEffects.forEach((effect) => {
      ...
      effectEl.addEventListener('mouseover', () => {
        showTooltip(
          `<strong>${effect.name}</strong><br>${effect.description || 'Zone effect active.'}<br><br>Turns Remaining: ${effect.duration}`
        );
      });
  ...
  gameState.pvpEncounter.playerStates.forEach((playerState) => {
    ...
    if (isActive && !playerState.isDead && !playerState.turnEnded) {
      ...
      timerBarEl.style.cssText =
        'width: 220px; height: 6px; background: #333; margin-top: 48px; border-radius: 3px; overflow: hidden;';
  ...
  offerings.forEach((spellName) => {
    ...
    if (canAfford) {
      card.querySelector('.training-learn-btn').addEventListener('click', () => {
        showConfirmationModal(`Learn ${spell.icon} ${spell.name} for ${trainingCost} QP?`, () => {
...
function renderDuelScreen() {
  ...
  const localPlayer =
    gameState.duelState.player1.id === socket.id ? gameState.duelState.player1 : gameState.duelState.player2;
  const opponent =
    gameState.duelState.player1.id === socket.id ? gameState.duelState.player2 : gameState.duelState.player1;
...
function renderZoneCards(cards) {
  ...
  cards.forEach((card, index) => {
    ...
    if (card.physicalResistance || card.magicalResistance) {
      ...
      if (card.physicalResistance) {
        resContainer.appendChild(
          buildBadge(
            '🛡️',
            card.physicalResistance,
            'Physical',
            `Reduces all Physical damage taken by ${card.physicalResistance}.<br><em>Use magic or elemental attacks to bypass!</em>`,
            '#bdc3c7'
          )
        );
      }
      if (card.magicalResistance) {
        resContainer.appendChild(
          buildBadge(
            '✨',
            card.magicalResistance,
            'Magical',
            `Reduces all Magical damage taken by ${card.magicalResistance}.<br><em>Use physical attacks to bypass!</em>`,
            '#6495ED'
          )
        );
      }
...
function createEffectsContainer(stateObject) {
  ...
  if (stateObject.reactions) {
    stateObject.reactions.forEach((reaction) => {
      ...
      const triggerText = Array.isArray(reaction.triggerOn) ? reaction.triggerOn.join(' or ') : reaction.triggerOn;
...
export function renderPlayerActionBars() {
  ...
  if (gameState.pvpEncounter) {
    ...
    localPlayerTurnEnded =
      gameState.activePhase !== 'player' ||
      gameState.partyMemberStates[gameState.activePlayerIndex]?.playerId !== localPlayerState.playerId ||
      localPlayerState.turnEnded;
  } else if (gameState.inDuel && gameState.duelState) {
    localPlayerTurnEnded =
      gameState.duelState.activePlayerId !== localPlayerState.id ||
      gameState.duelState.ended ||
      localPlayerState.turnEnded;
  }
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (spell) {
      ...
      if (
        spell.type === 'attack' ||
        spell.type === 'aoe' ||
        spell.type === 'versatile' ||
        spell.type === 'revive' ||
        spell.type === 'debuff' ||
        spell.type === 'cleanse' ||
        spell.type === 'cauterize' ||
        spell.type === 'expendHeat' ||
...
export function updateActionUI() {
  ...
  if (gameState.turnState.selectedAction) {
    ...
    if (
      action.type === 'attack' ||
      action.type === 'aoe' ||
      action.type === 'versatile' ||
      action.type === 'debuff' ||
      action.weaponDamage ||
      action.type === 'expendHeat'
    ) {
    ...
    if (
      ...
      action.type === 'versatile' ||
      action.type === 'cleanse' ||
      action.type === 'cauterize' ||
      action.type === 'expendHeat'
    ) {
      document
        .querySelectorAll('#party-cards-container .card.player:not(.dead), #zone-cards .card.player:not(.dead)')
        .forEach((playerCard) => {
...
export function showDebuffSelectionModal({ targetName, debuffs, maxSelectable, casterName: _casterName }) {
  import('../network.js').then((Network) => {
    ...
    document.getElementById('confirm-cleanse-btn').addEventListener('click', () => {
      const selected = Array.from(document.querySelectorAll('.debuff-checkbox:checked')).map((cb) =>
...
export function showBackpack() {
  ...
  goldDisplay.style.cssText =
    'text-align: right; max-width: 650px; margin: 8px auto 0; font-size: 1.1em; color: #f1c40f;';
...
export function showCompostMenu() {
  ...
  const compostableItems = gameState.inventory
    .map((item, originalIndex) => ({ item, originalIndex }))
    .filter((entry) => isCompostable(entry.item));
  ...
  if (compostableItems.length === 0) {
    ...
    compostableItems.forEach(({ item, originalIndex }) => {
      ...
      const iconHTML = item.icon
        ? item.icon.includes('/')
          ? `<img src="${item.icon}" style="width:32px;height:32px;object-fit:cover;border-radius:4px;">`
...
export function showCharacterSheet() {
  ...
  let resistancesHTML = '';
  if (calculatedStats.physicalResistance > 0)
    resistancesHTML += `<p>🛡️ Physical: ${calculatedStats.physicalResistance}</p>`;
  if (calculatedStats.magicalResistance > 0)
    resistancesHTML += `<p>✨ Magical: ${calculatedStats.magicalResistance}</p>`;
...
export function showGroundLootModal() {
  ...
  modalContentEl.innerHTML =
    '<h2>Ground Loot & Inventory</h2><p>Take items from the ground or drop items from your inventory to make space.</p>';
...
export function showCampfireCookingModal() {
  ...
  if (cookingRecipes.length === 0) {
    ...
    cookingRecipes.forEach((recipe) => {
      const canCraft = Object.entries(recipe.materials).every(([mat, qty]) => (materialCounts[mat] || 0) >= qty);
      ...
      row.style.cssText =
        'display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; background: rgba(255,255,255,0.05); border-radius: 8px;';
      row.innerHTML = `
