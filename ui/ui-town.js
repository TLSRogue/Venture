...
export function renderMerchant() {
  ...
  const permanentStock = gameData.allItems.filter(
    (item) => item.type === 'tool' || item.name === 'Spices' || item.permanentMerchantStock === true
  );
  ...
  permanentStock.forEach((item) => {
    ...
    itemEl.addEventListener('mouseover', () =>
      showTooltip(buildItemTooltip(item, { showPrice: true, action: 'Click to Buy' }))
    );
  ...
  quickActionsContainer.style.cssText =
    'margin-top: 10px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 4px;';
...
export function showCraftingModal(recipeIndex) {
  ...
  for (const materialName in recipe.materials) {
    ...
    const playerAmount =
      gameState.inventory.filter((i) => i && i.name === materialName).reduce((sum, i) => sum + (i.quantity || 1), 0) +
...
export function renderTrainer() {
  ...
  spellsToDisplay.forEach((spell) => {
    ...
    const knowsSpell =
      gameState.spellbook.some((s) => s.name === spell.name) ||
      gameState.equippedSpells.some((s) => s.name === spell.name);
