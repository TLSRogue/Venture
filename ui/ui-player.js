...
export function updateDisplay() {
  ...
  if (adventureHUD && (gameState.currentZone || gameState.inDuel)) {
    ...
    if (gameState.pvpEncounter && gameState.pvpEncounter.playerStates) {
      ...
      localPlayerState =
        gameState.duelState.player1.id === Network.socket?.id
          ? gameState.duelState.player1
          : gameState.duelState.player2;
      if (localPlayerState) {
        currentHealth = localPlayerState.health;
        currentMaxHealth = localPlayerState.maxHealth;
        currentAP = localPlayerState.actionPoints;
    ...
    try {
      ...
      console.error('Error determining shield value', e);
    }
...
export function renderSpells() {
  ...
  if (gameState.partyId && gameState.partyMemberStates) {
    ...
    const localPlayerState =
      gameState.duelState.player1.id === Network.socket?.id ? gameState.duelState.player1 : gameState.duelState.player2;
...
export function showGemSocketModal(equipmentSlot) {
  ...
  if (gemsInInventory.length === 0) {
    ...
    gemsInInventory.forEach(({ gem, index }) => {
      const bonusText = Object.entries(gem.gemBonus || {})
  ...
  modalContent +=
    '<div class="action-buttons" style="margin-top: 20px;"><button class="btn" onclick="document.getElementById(\'modal\').classList.add(\'hidden\')">Cancel</button></div>';
  showModal(modalContent);
}
...
function generateQuestLogHTML() {
  ...
  if (gameData.cardPools) {
    Object.values(gameData.cardPools)
      .flat()
      .forEach((poolItem) => {
  ...
  activeQuests.forEach((quest) => {
    ...
    if (quest.status === 'readyToTurnIn') {
      ...
      for (const itemName in quest.details.turnInItems) {
        ...
        const currentAmount = gameState.inventory
...
export function showQuestLogModal() {
  const content =
    generateQuestLogHTML() +
    `<div class="action-buttons" style="margin-top: 20px;"><button class="btn" onclick="document.getElementById('modal').classList.add('hidden')">Close</button></div>`;
  showModal(content);
}
...
window.abandonQuest = (questId) => {
  if (
    confirm(
      'Are you sure you want to abandon this quest? Any progress will be lost and you will need to accept it again.'
    )
  ) {
