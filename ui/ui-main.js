...
export function buildItemTooltip(item, options = {}) {
    ...
    if (item.type === 'weapon' || item.type === 'shield') {
        const damageTypeColors = {
            'Physical': '#ddd', 'Fire': '#e74c3c', 'Frost': '#3498db',
            'Arcane': '#9b59b6', 'Holy': '#f1c40f', 'Nature': '#2ecc71', 'Shadow': '#8e44ad'
        };
...
function buildComparisonSection(newItem, equippedItem) {
    ...
    const allStats = new Set([
        ...
        ...Object.keys(equippedItem.bonus || {})
    ]);
...
export function buildSpellTooltip(spell, cooldownRemaining = 0) {
    ...
    // School colors
    const schoolColors = {
        'Fire': '#e74c3c', 'Frost': '#3498db', 'Holy': '#f1c40f',
        'Shadow': '#8e44ad', 'Nature': '#2ecc71', 'Arcane': '#9b59b6',
        'Physical': '#ddd', 'Combat': '#e67e22'
    };
    const schoolColor = schoolColors[spell.school] || '#888';

    // Type colors
    const typeColors = {
        'attack': '#e74c3c', 'heal': '#2ecc71', 'buff': '#3498db',
        'versatile': '#9b59b6', 'revive': '#f1c40f', 'utility': '#888'
    };
